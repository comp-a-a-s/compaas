...
def test_build_run_status_payload_sanitizes_noisy_agent_task():
    run = {
        "id": "run-noise",
        "project_id": "proj-1",
        "status": "executing",
        "updated_at": _iso_seconds_ago(3),
    }
    workforce_snapshot = {
        "workers": [
            {
                ...
                "agent_name": "Marcus",
                "state": "working",
                "task": "1→# Mathics 2→ 3→This workspace was generated by COMPaaS. 4→ 5→Use this folder",
            }
        ]
    }
    ...
    assert len(payload["active_agents"]) == 1
    task_text = str(payload["active_agents"][0].get("task", ""))
    assert "→" not in task_text
    assert "Mathics" in task_text
